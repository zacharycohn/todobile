...
This schema is intended for Supabase Postgres and should be managed through versioned SQL migrations in the repository.
...
This table exists because the API contract includes device push-token registration and notification delivery targeting.
...
These indexes are required by the API contract and expected usage patterns.
...
Enable RLS on all application tables. Even if the backend uses the service role for some operations, policies are still required for direct client access, testing, and defense in depth.
